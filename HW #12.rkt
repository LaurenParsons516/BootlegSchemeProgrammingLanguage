...
(define block-expression-parser
  (lambda (block-expression)
    (cond
      ((eq? (car block-expression) 'do)
       (list 'block-exp (map (lambda (exp)
                               (no-parser exp)) (cdr block-expression))))
  ...
  (lambda (parsed-block-expression env)
    (cond
      ((eq? (car parsed-block-expression) 'block-exp)
       (map (lambda (exp)
              (run-parsed-code exp env)) (cadr parsed-block-expression))))))
...
(define sample-no-code '(do (do-mathy-stuff + a 2) (do-mathy-stuff + b 9) (do-mathy-stuff * a b)))
(display (run-parsed-code (no-parser sample-no-code) env))