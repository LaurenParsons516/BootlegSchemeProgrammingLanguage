...
(define no-parser
  (lambda (no-code)
    (cond
      ...
      ((eq? (car no-code) 'question)
       (list 'bool-exp (no-parser (cadr no-code)) (no-parser (cadddr no-code)) (no-parser (cadddr (cddr no-code)))))
...
(define sample-no-code '(call (function (x) x) (question 1 do 8 otherwise 5)))
  ...
  (lambda (parsed-no-code env)
    (cond
      ...
      ((eq? (car parsed-no-code) 'bool-exp)
       (if (eq? (run-parsed-code (cadr parsed-no-code) env) 1)
           (run-parsed-code (caddr parsed-no-code) env)
           (run-parsed-code (cadddr parsed-no-code) env)))         
      (else
       (run-parsed-code
