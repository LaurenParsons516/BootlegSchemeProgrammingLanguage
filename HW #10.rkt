...
(define env-let-get-var-values
  (lambda (lst env)
    (if (null? lst)
        '()
        (cons (run-parsed-code (no-parser (cadar lst)) env) (env-let-get-var-values (cdr lst) env)))))
    ...
    (cond
      ...
      ((eq? (car parsed-no-code) 'let-exp)
       (let* ((var-2-lists (env-let-mapper (cadr parsed-no-code) env))
              (new-env (extend-env (extend-scope (car var-2-lists) (cadr var-2-lists) '()) env))
...
(define sample-no-code '(local-vars ((a 2) (b 3)) (local-vars ((a b) (d 5)) (do-mathy-stuff + a d))))
